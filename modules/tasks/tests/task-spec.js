(function(angular, moment, chance) {
    'use strict';
    describe('Tasks Feature', function() {
        ...
        if (_runUserStory === null || _runUserStory === 1) {
            describe('1. As a user, I want to add a task so that the work to be ' +
                'completed on a project or appointment can be managed. (MVP)',
                function() {
                    // the add task dom element
                    var _element;

                    function createAddTaskDomElement() {
                        _element = $compile('<ls-add-task></ls-add-task>')(scope);
                        scope.$digest();
                    }
                    ...
                    it('a. Tasks have a text description',
                        function() {
                            createAddTaskDomElement();
                            setAddDescription(_description); // 513 characters
                            expect(getDescriptionTextArea().val() === _description)
                                .toBeTruthy();
                        });

                    it('b. An error message "Please shorten the task description" is ' +
                        'displayed when the entered description exceeds 512 characters',
                        function() {
                            createAddTaskDomElement();
                    ...
                    it('c. A character count is displayed showing the entered length ' +
                        'and max length.',
                        function() {
                            createAddTaskDomElement();
                    ...
                    it('d. At least one character is required in the description to ' +
                    ...
                    it('e. Pressing RETURN on the keyboard does nothing when there is ' +
                        'no entered text description.',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('f. Pressing RETURN on the keyboard adds the task when there ' +
                        'is a text description.',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('g. Pressing add button adds the task when there is a text ' +
                        'description.',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('h. Pressing ESCAPE on the keyboard clears the description.',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('i. The added task is sent to the server on add.',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('j. The information sent to the server is: task description, ' +
                        'parent id, and parent type (appointment, project)',
                        function() {
                            createAddTaskDomElement();
                        });

                    it('k. The server responds to the add with a task to add to the ' +
                        'task list.  (see view task)',
                        function() {
                            createAddTaskDomElement();
                        });
                });
        ...
        if (_runUserStory === null || _runUserStory === 3) {
            describe('3. As a user, I want to edit a task description so that the task ' +
                'list is flexible. (MVP)',
                function() {
                    // the add task dom element
                    var _element;

                    if (_runAcceptanceCriteria === null || _runAcceptanceCriteria === 'h') {
                        it('a. An error message “Please shorten the task '+
                            'description” is displayed when the entered '+
                    ...
                    if (_runAcceptanceCriteria === null || _runAcceptanceCriteria === 'h') {
                        ...
                        it('b. A character count is displayed showing the entered length and max length.',
                            function() {
        ...
        if (_runUserStory === null || _runUserStory === 4) {
            describe('4. As a user, I want to delete a task so I can remove ' +
                'unnecessary tasks. (MVP)',
                function() {
                    // the add task dom element
                    var _element;
        ...
        if (_runUserStory === null || _runUserStory === 5) {
            describe('5. As a user, I want to have the ability to differentiate ' +
                'my tasks by color so that the tasks can be categorized. (MVP)',
                function() {
                    // the add task dom element
                    var _element;
        ...
        if (_runUserStory === null || _runUserStory === 6) {
            describe('6. As a user, I want to mark my tasks as complete or ' +
                'incomplete, so that I and others can view the status of completed ' +
                'work. (MVP)',
                function() {
                    // the add task dom element
                    var _element;
        ...
        if (_runUserStory === null || _runUserStory === 7) {
            describe('7. As a user I want all of my actions and updates to my tasks ' +
                'sent to the server for to record date, time and GPS location so ' +
                'that later myself and others can understand when and where changes ' +
                'happened. (MVP)',
                function() {
                    // the add task dom element
                    var _element;
        ...
        if (_runUserStory === null || _runUserStory === 8) {
            describe('8. As a user, I want to sort, search and filter tasks so ' +
                'that tasks can be reviewed. (MVP)',
                function() {
                    // the add task dom element
                    var _element;
